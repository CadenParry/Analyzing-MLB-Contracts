...
function readData(){
    ...
    d3.csv("robinson-cano-stats.csv", d3.autoType).then(data => {
        initialize(data);
    });
}
...
function createLinePlot(data){
    ...
    var selected_players = ['Robinson Cano'];
    ...
    .domain(d3.extent(filtered, (_, i) => filtered[i].Year))
    .range([marginLeft, width - marginRight]);
    const yscale = d3.scaleLinear([0, d3.max(filtered, (_, i) => filtered[i].WAR)], [height - marginBottom, marginTop]);
